...
def simulate_llm_interim(transcript, round_number):
    ...
    interim = (
        f"After {round_number} round(s), the evidence shows: {combined[:100]}..."
    )
    return interim
...
@app.route('/')
def index():
    return "Courtroom Showdown Server is running."
...
def start_game(join_code):
    session = sessions[join_code]
    scenario = simulate_llm_scenario()
    ...
    active_role = session['turn_order'][session['submissionCount'] % 2]  # submissionCount is 0 initially
    socketio.emit('turnUpdate', {
...
def handle_submit_evidence(data):
    ...
    print(f"Submission from {role} in session {join_code} (Round {current_round}): {evidence_text}")
